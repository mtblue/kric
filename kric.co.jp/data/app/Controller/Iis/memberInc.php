...
*/
	function pageErrorCheck() {
		...
		if (isset($this->form['login_id']) == false) {
			$md5 = md5( date("YmdD His") );
			//先頭から 10 文字取得
			$str = substr($md5, 0, 10);
			$this->form['login_id']=$str;
		}
		if (isset($this->form['pass']) == false) {
			$md5 = md5(md5( date("YmdD His")));
			//先頭から 10 文字取得
			$str = substr($md5, 0, 10);
			$this->form['pass'] = $str;
		}
		...
		// メールアドレス重複チェック
		if (BASE_NAME == 'member_new') {

			for($i=1; $i<=3; $i++) {
				if (isset($objOrg->form['email'.$i]) == false || (isset($objOrg->form['email'.$i]) == true && $objOrg->form['email'.$i] == '')) { continue; }
				// 情報取得
				$TAKE = "COUNT(*)";
				// DBテーブル設定
				$TABLE = "member";
				...
				$WHERE .= " AND (";
				$WHERE .= " (member.email1 IS NOT NULL AND member.email1 = '?')";
				$WHERE .= " OR (member.email2 IS NOT NULL AND member.email2 = '?')";
				$WHERE .= " OR (member.email3 IS NOT NULL AND member.email3 = '?')";
				$WHERE .= " )";
				$WHERE_VAL = array($objOrg->form['email'.$i], $objOrg->form['email'.$i], $objOrg->form['email'.$i]);
				// クエリ実行
				$D_NUM = $objOrg->Select_wide_use(0, $TAKE, $TABLE, $WHERE, $WHERE_VAL);
				if ($D_NUM <= 0) { $D_NUM = 0; }
				if ($D_NUM > 0) {
					$objOrg->errDoc[] = $objOrg->error['dami'] = $objOrg->formList['email'.$i]['name'].'は既に登録されています';
				}
			}

		} elseif (BASE_NAME == 'member_edit') {
			...
			for($i=1; $i<=3; $i++) {
				...
				$WHERE .= " OR (member.email3 IS NOT NULL AND member.email3 = '?')";
				$WHERE .= " )";
				$WHERE .= " AND member.member_id != ".$objOrg->form['id'];
				$WHERE_VAL = array($objOrg->form['email'.$i], $objOrg->form['email'.$i], $objOrg->form['email'.$i]);
				// クエリ実行
				$D_NUM = $objOrg->Select_wide_use(0, $TAKE, $TABLE, $WHERE, $WHERE_VAL);
				if ($D_NUM <= 0) { $D_NUM = 0; }
		...
		for($i=1; $i<=3; $i++) {
			if (isset($objOrg->form['email'.$i]) == false || (isset($objOrg->form['email'.$i]) == true && $objOrg->form['email'.$i] == '')) { continue; }
			if (in_array($objOrg->form['email'.$i], $tmpMail) == true) {
				$objOrg->errDoc[] = $objOrg->error['dami'] = 'メールアドレスが重複しています';
				break;
			} else {
				$tmpMail[] = $objOrg->form['email'.$i];
			}
		}
